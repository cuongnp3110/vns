...
if(isset($_POST['invoiceEdit'])){
    ...
    if($validate == true){
        $result = mysqli_query($conn, "select supplier_id, total, payment from invoice_im where invoice_im_id = '".$_GET['id']."'");
        ...
        $result = mysqli_query($conn, "select * from invoice_im_item where invoice_im_id = '".$_GET['id']."'");
        ...
        mysqli_query($conn, "DELETE FROM invoice_im_item WHERE invoice_im_id = '".$_GET['id']."' ");
    
        mysqli_query($conn, "
        UPDATE invoice_im SET 
        supplier_id = '".$_POST['sup']."', 
        discount = '".$_POST['discount']."',
        subtotal = '".$_POST['subtotal']."',
        total =  '".$_POST['total']."',
        payment = '".$_POST['paid']."',
        note = '".$_POST['note']."'
        WHERE invoice_im_id = '".$_GET['id']."' ");

        $lastInsertId = mysqli_insert_id($conn);
        for ($i = 0; $i < count($_POST['productCode']); $i++) {
    
            mysqli_query($conn, "
            INSERT INTO invoice_im_item(invoice_im_id, product_code, product_name, quantity, price, total, account_id) 
        ...
        mysqli_query($conn, "insert into log(log, account_id) values('By ".$_SESSION['email']." | Invoice Import Add (Total Price): $".$_POST['total']."','".$_SESSION['account_id']."')");
        
        mysqli_query($conn, "
        UPDATE supplier SET purchased = purchased + '".$_POST['total']."', debt = debt + '".$_POST['debt']."'
        WHERE supplier_id = '".$_POST['sup']."' and account_id = '".$_SESSION['account_id']."'");
    
        echo '<meta http-equiv="refresh" content="0;URL =?page=invoice_im"';
    } else {
        $err = "invoiceEmpty";
    }
...
<div class="container-fluid" id="container-wrapper" style="display: flex; justify-content: center;">
    <div class="col-lg-12">
        ...
        <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                <h6 class="m-0 font-weight-bold" style="color: #fc544b"><i class="fas fa-fw fa-pen"></i> Edit Invoice for Import Products</h6>
            </div>
                        ...
                        <div class="row" style="margin-top: 15px">
                            ...
                            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-3">
                                <div class="row">
                                    ...
                                    <div class="col-md-5" style="text-align: left ; padding: 20px 5px 5px 5px"><b>Supplier:</b></div>
                            ...
                            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-3">
                                <?php
                                    $result = mysqli_query($conn, "select discount, subtotal, total, payment, note from invoice_im where invoice_im_id ='$invoiceImId'");
                                    $row = mysqli_fetch_array($result, MYSQLI_ASSOC)
                                ?>
                                <div class="row">
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-2" style="padding-bottom: 0" >
                                <div class="form-group" style="position: absolute; bottom: 0; right: 10px">
                                    <input  style="background-color: #fc544b; border-color: #fc544b" data-loading-text="Saving Invoice..." type="submit" name="invoiceEdit" value="Save Invoice" class="btn btn-success submit_btn invoice-save-btm">						
                                </div>
                            </div>
                        </div>
                    </div>
